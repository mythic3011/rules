...
def service_region_groups(service: dict[str, object]) -> list[str]:
    regions = cast(list[str], service["regions"])
    return [GROUP[region] for region in regions] + [GROUP["other"]]
...
def render_ini() -> str:
    ...
    def emit_clusters(clusters: list[list[str]]) -> None:
        """Ruleset clusters separated by exactly one blank line."""
        for index, cluster in enumerate(clusters):
            if index:
                blank()
            lines.extend(cluster)

    def blank_separated(items: list[str]) -> list[str]:
        """Return items with exactly one blank line between each."""
        result: list[str] = []
    ...
    def emit_section(content: list[str], title: str, subtitle: str | None = None) -> None:
        """Two leading blanks, banner (with trailing blank), then content lines."""
        blank()
        blank()
        lines.extend(ini_banner(title, subtitle))
        lines.extend(content)

    blank()
    lines.extend(render_ini_mvp_rules(ini_mvp["rules"]["beforeLegacy"]))

    in_merged_cluster = False
    for service in AI_SERVICES:
        if service["id"] in ini_mvp["migration"]["legacyReplacementIds"]:
            continue
        if not in_merged_cluster:
            blank()
        if service["payload"]:
            lines.append(
        ...
        if service["id"] == "gemini":
            # Gemini..Poe share one cluster: no blank between them.
            in_merged_cluster = True
        elif service["id"] == "poe":
            in_merged_cluster = False

    legacy_after = render_ini_mvp_rules(ini_mvp["rules"]["afterLegacy"])
    blank()
    lines.extend(legacy_after[:2])
    ...
    emit_clusters(
        [
            ...
            # GeoIP HK and FINAL are separate single-rule clusters.
            [
    ...
    emit_section(
        ...
        "Level 0 — Foundation groups",
        "Define these before anything references them",
    )

    # Level 1 — Automatic region groups (url-test).
    health_check = "`https://cp.cloudflare.com/generate_204`300,,50"
    auto_lines: list[str] = []
    for region in PRIMARY_REGION_ORDER:
        auto_lines.append(
            f"custom_proxy_group={GROUP[region]}`url-test`{REGION_FILTERS[region]}{health_check}"
        )
    auto_lines.append(
        f"custom_proxy_group={GROUP['other']}`url-test`{OTHER_REGION_FILTER}{health_check}"
    )
    emit_section(
        blank_separated(auto_lines),
        "Level 1 — Automatic region groups",
    )

    # Level 1 — Stable manual region groups from the plan (REJECT-first node filters).
    stable_lines = render_ini_mvp_groups(ini_mvp["groups"])
    emit_section(
        blank_separated(stable_lines[:3]),
        "Level 1 — Stable manual region groups",
    )

    # Level 2 — Shared routing selectors.
    emit_section(
        blank_separated([render_ini_global_auto_group(), render_ini_manual_group()]),
        "Level 2 — Shared routing selectors",
    )

    # Level 3 — AI service selectors. ChatGPT is fail-closed: it only offers
    # REJECT plus the manual selector, so the user must opt in explicitly.
    ai_selector_lines = [
        "; ChatGPT is fail-closed.",
        "; User must explicitly select 手動選擇.",
        "custom_proxy_group=🤖 ChatGPT`select`[]⛔ 拒絕`[]🚀 手動選擇",
    ]
    for service in AI_SERVICES:
        ...
        if service["direct_relaxed"]:
            candidates.append(GROUP["direct"])
        candidates.extend(service_region_groups(service))
        candidates.append(GROUP["reject"])
        ai_selector_lines.append(
            f"custom_proxy_group={service['group']}`select`{ini_group_candidates(candidates)}"
        )
    # Only the selector lines are blank-separated; the fail-closed comment
    # block stays contiguous with the ChatGPT line.
    emit_section(
        ai_selector_lines[:3] + [""] + blank_separated(ai_selector_lines[3:]),
        "Level 3 — AI service selectors",
        "One service = one visible group",
    )

    # Account-protected service and stable-session services come straight from
    # the plan (everything after the three Stable region groups).
    emit_section([stable_lines[3]], "Account-protected service")

    emit_section(
        blank_separated(stable_lines[4:]),
        "Stable-session / explicitly separated AI services",
    )

    # Level 4 — Final catch-all selector.
    emit_section(
        [
...
class IniMvpPlan(TypedDict):
    """Declared shape of generated/ai-routing/hk.ini-mvp-plan.json (validated at load)."""
