...
function describe(result) {
	...
	return observed + ' | ' + _('expected') + ': ' + expected + ' | ' + _('region') + ': ' + (result.regionMatch || 'unknown');
}
	...
	render: function() {
		...
		var body = services.map(function(service) {
			...
			button.addEventListener('click', ui.createHandlerFn(this, function() {
				...
				return callTrace(service.id).then(function(result) {
					resultCell.textContent = describe(result);
				});
			}));
		...
		runAll.addEventListener('click', ui.createHandlerFn(this, function() {
			...
			return Promise.all(services.map(function(service) {
				return callTrace(service.id).then(function(result) {
					rows[service.id].textContent = describe(result);
				});
			}));
		}));
		...
		return E('div', {}, [
			...
			E('p', {}, _('The requests originate on the router through fixed service trace endpoints. They show observed public egress and Cloudflare edge data. Source-based policies for forwarded LAN clients can differ, so a region match is evidence, not a full path proof.')),
			E('p', {}, runAll),
			E('table', { 'class': 'table' }, [
