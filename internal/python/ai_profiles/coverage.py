...
def _service_geosites(catalog: Catalog, service_id: str) -> set[str]:
    service = next((service for service in catalog.services if service.id == service_id), None)
    if service is None:
        return set()
    return {
        source.value
...
def audit_rule_coverage(
    ...
    *,
    ...
    for service_id, geosites in expected.items():
        service = next((service for service in catalog.services if service.id == service_id), None)
        if service is None:
            findings.append(CoverageFinding("error", "missing-aggregate", f"Missing aggregate AI service: {service_id}"))
            continue
    ...
    if spec and spec.upstream_snapshot_file:
    ...
    if ini_plan_path.exists():
        try:
            plan = json.loads(ini_plan_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            findings.append(CoverageFinding("error", "invalid-ini-plan", f"Cannot inspect INI MVP plan: {exc}"))
        else:
            ...
            if not required_ini_geosites.issubset(ai_other_geosites):
                findings.append(
                    CoverageFinding(
                        "warning",
            ...
            try:
                ...
                findings.append(CoverageFinding("error", "invalid-upstream-manifest", str(exc)))
            else:
