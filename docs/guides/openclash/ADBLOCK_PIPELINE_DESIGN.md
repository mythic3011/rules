...
- source definitions live in `internal/config/filtering/sources/*.yaml`
- output wiring lives in `internal/config/filtering/categories.yaml`
- policy and vendor exceptions live in `internal/config/filtering/domain-policies.yaml`
- local overrides live in `dns/local_*list.txt`
- generation logic lives in `internal/python/generate_adblock_outputs.py`
...
1. GitHub Actions or local run executes `internal/python/generate_adblock_outputs.py`.
2. The generator reads category metadata from `internal/config/filtering/categories.yaml`.
...
8. OpenWrt pulls generated outputs through `setup/openclash/scripts/apply_adblock_dnsmasq.sh`.
9. Optional cron installs recurring refresh via `setup/openclash/scripts/setup_adblock_cron.sh`.
...
- `internal/config/filtering/categories.yaml` defines category shape
- `internal/config/filtering/sources/custom/*.yaml` provides extension points
