...
MANIFEST_URL="${MANIFEST_URL:-${CDN_BASE}/setup/openclash/scripts/manifests/adblock.json}"
...
ALLOW_ETC_HOSTS_MERGE="${ALLOW_ETC_HOSTS_MERGE:-0}"
...
hosts_to_dnsmasq_conf() {
    title="$1"
    source_file="$2"
    target_file="$3"

    {
        echo "# Generated by setup/openclash/scripts/apply_adblock_dnsmasq.sh"
        echo "# SOURCE: ${title}"
        echo "# FORMAT: dnsmasq conf"
        awk '
            /^[[:space:]]*($|#)/ { next }
            NF < 2 { next }
            {
                for (i = 2; i <= NF; i++) {
                    domain = tolower($i)
                    sub(/\r$/, "", domain)
                    if (domain ~ /^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?(\.[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)+$/) {
...
guard_hosts_target() {
    if [ "$HOSTS_TARGET_FILE" = "/etc/hosts" ] && [ "$ALLOW_ETC_HOSTS_MERGE" != "1" ]; then
        log "/etc/hosts merge is disabled; use dnsmasq conf outputs or set HOSTS_TARGET_FILE to a dnsmasq addn-hosts file"
        return 1
    fi
}
...
apply_remote_hosts_dnsmasq_confs() {
    if [ "$ENABLE_ADOBE_REMOTE" = "1" ]; then
        log "converting remote Adobe hosts into dnsmasq conf"
        downloaded="$(download_to_temp "$ADOBE_REMOTE_URL")"
        hosts_to_dnsmasq_conf "adobe-remote" "$downloaded" "$TARGET_DIR/rules-adobe-remote.dnsmasq.conf"
        rm -f "$downloaded"
    fi
}
