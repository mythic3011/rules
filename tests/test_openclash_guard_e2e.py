...
"""
    ...
    def test_full_built_bundle_lifecycle(self) -> None:
        ...
        self.assertEqual(status_payload["runtime"]["source"], "local")
        self.assertEqual(status_payload["rules"]["activation"], "staged")
        self.assertEqual(status_payload["firewall"]["table"], "absent")
        self.assertNotEqual(empty_list.stdout.strip(), status_payload["rules"]["activation"])
        ...
        after_apply = json.loads(self.run_guard("status", "--json").stdout)
        self.assertEqual(after_apply["firewall"]["table"], "active")
        self.assertEqual(after_apply["rules"]["activation"], "staged")
        self.assertEqual(after_apply["enforcement"], status_payload["enforcement"])
        ...
        after_activate = json.loads(self.run_guard("status", "--json").stdout)
        self.assertEqual(after_activate["rules"]["activation"], "active")
        self.assertEqual(after_activate["firewall"]["table"], "active")
        health_active = self.run_guard("health-check")
