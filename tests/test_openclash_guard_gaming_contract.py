...
class OpenClashGuardGamingContractTests(unittest.TestCase):
    ...
    def test_uci_multi_client_list_uses_real_newline_delimiter(self) -> None:
        script = f"""
set -eu
. "{GAMING}"
...
uci() {{
    ...
    shift 2
    [ "$1" = -q ] || exit 31
    printf '10.0.0.11%s10.0.0.12\\n' "$_delimiter"
}}

_guard_game_ips=$(guard_game_src_ips)
_guard_game_ip_in 10.0.0.11 $_guard_game_ips || exit 32
_guard_game_ip_in 10.0.0.12 $_guard_game_ips || exit 33
"""
...
'''
    ...
    def test_renderer_has_directional_rules_and_direct_egress_constraint(self) -> None:
        ...
        self.assertIn("guard_dataplane_capability_mark", text)
        self.assertIn("meta mark != $_guard_gr_cap", text)
        self.assertIn("meta mark $_guard_gr_cap", text)
        self.assertIn("meta mark set 0 accept", text)
