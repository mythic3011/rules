...
test("Mihomo projection resolves external upstream sources from the shared lock manifest", async () => {
  const projection = await loadMihomoProjectionConfig("internal/config/ai-routing/mihomo.yaml");
  const manifest = JSON.parse(await readFile("internal/config/ai-routing/upstream-sources.json", "utf8")) as {
    sources: Record<string, { label: string; repository: string; revision: string; rawBaseUrl: string }>;
  };
...
test("VPSDance revision is declared once in the shared manifest, not inline in Mihomo YAML", async () => {
  ...
  const document = YAML.parse(text) as { sources: Record<string, unknown>; upstreamSourceManifest?: string };
  assert.equal(document.upstreamSourceManifest, "ai-routing/upstream-sources.json");
