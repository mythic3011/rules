...
test("shadow template rejects missing, unknown, and duplicate named slots", () => {
  assert.throws(() => renderShadowTemplate("${header}", SHADOW_TEMPLATE_CONTEXT), ShadowTemplateError);
  assert.throws(() => renderShadowTemplate("${header}\n${unknown}\n${static-top-level}\n${proxy-providers}\n${dns}\n${proxy-groups}\n${rules}\n${rule-providers}", SHADOW_TEMPLATE_CONTEXT), ShadowTemplateError);
  assert.throws(() => renderShadowTemplate("${header}\n${unknown_slot}\n${static-top-level}\n${proxy-providers}\n${dns}\n${proxy-groups}\n${rules}\n${rule-providers}", SHADOW_TEMPLATE_CONTEXT), ShadowTemplateError);
  assert.throws(() => renderShadowTemplate("${header}\n${header}\n${static-top-level}\n${proxy-providers}\n${dns}\n${proxy-groups}\n${rules}\n${rule-providers}", SHADOW_TEMPLATE_CONTEXT), ShadowTemplateError);
});
...
async function composeWithBase(contents: string, mutateParity?: (value: Record<string, unknown>) => void): Promise<void> {
  ...
  try {
    ...
    const mutable = structuredClone(parity) as unknown as Record<string, unknown>;
    mutateParity?.(mutable);
    await composeShadowProfile(config, projection, base, mutable as typeof parity, SHADOW_TEMPLATE);
  } finally {
...
test("shadow composer applies only four exact typed deltas and preserves closed unrelated inventory", async () => {
  const { config, projection, parity } = await shadowInputs();
  const result = await composeShadowProfile(config, projection, RELAXED_BASE, parity, SHADOW_TEMPLATE);
  ...
  assert.equal(groups.some((group) => group.name === "🔐 Claude US Pinned"), false);
  assert.deepEqual((groups.find((group) => group.name === "🔐 Claude Account Guard")?.proxies), ["REJECT"]);
  ...
  (injectedObject["proxy-groups"] as Array<unknown>).unshift({ name: "🤖 Unrelated", type: "select", proxies: ["DIRECT"] });
  (injectedObject.rules as Array<unknown>).unshift("RULE-SET,AI_Unrelated_Classical,🤖 Unrelated");
  (injectedObject["rule-providers"] as Record<string, unknown>).AI_Unrelated_Classical = { type: "http", behavior: "classical", url: "https://example.invalid/rules.yaml", path: "./rule/AI_Unrelated_Classical.yaml" };
  ...
  try {
    ...
    const preserved = await composeShadowProfile(config, projection, basePath, parity, SHADOW_TEMPLATE);
    assert.match(preserved.candidateYaml, /AI_Unrelated_Classical/);
    assert.match(preserved.candidateYaml, /🤖 Unrelated/);
    assert.match(preserved.candidateYaml, /RULE-SET,AI_Unrelated_Classical,🤖 Unrelated/);
...
test("shadow composer rejects stale, missing, and unapproved typed delta contracts", async () => {
  ...
  await assert.rejects(
    () => composeWithBase(base, (manifest) => { const deltas = manifest.deltas as Record<string, unknown>; const groups = deltas["proxy-groups"] as Record<string, unknown>; (groups.add as string[]).pop(); }),
    (error: unknown) => error instanceof ShadowProfileError && error.issues.some((entry) => entry.path.join(".") === "deltas.proxy-groups.add"),
  );
  const dnsPresent = YAML.parse(base) as Record<string, unknown>;
  ((dnsPresent.dns as Record<string, unknown>)["nameserver-policy"] as Record<string, unknown>)["rule-set:AI_Claude_Classical"] = ["https://1.1.1.1/dns-query#🔐 Claude Account Guard"];
  await assert.rejects(
    () => composeWithBase(YAML.stringify(dnsPresent)),
    (error: unknown) => error instanceof ShadowProfileError && error.issues.some((entry) => entry.path.join(".") === "dns.nameserver-policy.rule-set:AI_Claude_Classical"),
  );
  await assert.rejects(
    () => composeWithBase(base, (manifest) => { const deltas = manifest.deltas as Record<string, unknown>; const rules = deltas.rules as Record<string, unknown>; (rules.add as string[]).push("RULE-SET,AI_Unapproved_Classical,DIRECT"); }),
    (error: unknown) => error instanceof ShadowProfileError && error.issues.some((entry) => entry.path.join(".") === "deltas.rules.add"),
  );
  await assert.rejects(
    () => composeWithBase(base, (manifest) => { const deltas = manifest.deltas as Record<string, unknown>; delete deltas.dns; }),
...
test("shadow composer rejects a compiled group collision with preserved legacy inventory", async () => {
  const base = YAML.parse(await readFile(RELAXED_BASE, "utf8")) as Record<string, unknown>;
  (base["proxy-groups"] as Array<unknown>).unshift({ name: "🤗 Hugging Face", type: "select", proxies: ["DIRECT"] });
  await assert.rejects(
    () => composeWithBase(YAML.stringify(base)),
    (error: unknown) => error instanceof ShadowProfileError && error.issues.some((entry) => entry.message.includes("collides with preserved")),
  );
});
...
test("shadow composer refuses changed unowned surfaces, interval anomalies, and malformed boundaries", async () => {
  const base = await readFile(RELAXED_BASE, "utf8");
  const { config, projection, parity } = await shadowInputs();
  const baseline = await composeShadowProfile(config, projection, RELAXED_BASE, parity, SHADOW_TEMPLATE);
  const baseObject = await loadLegacyRelaxedBase(RELAXED_BASE);
  const fragment = YAML.parse(renderMihomoFragment(compileMihomoFragment(config, projection, "hk"))) as Record<string, unknown>;
  const changedStatic = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; changedStatic.port = 9999;
  assert.throws(() => validateShadowParityCandidate(baseObject, changedStatic, fragment, parity), ShadowProfileError);
  const changedProxyProvider = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; ((changedProxyProvider["proxy-providers"] as Record<string, Record<string, unknown>>).provider1 ?? {}).url = "https://example.invalid/changed-subscription";
  assert.throws(() => validateShadowParityCandidate(baseObject, changedProxyProvider, fragment, parity), ShadowProfileError);
  const extraRoot = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; extraRoot.unapprovedTopLevel = true;
  assert.throws(() => validateShadowParityCandidate(baseObject, extraRoot, fragment, parity), ShadowProfileError);
  const changedGroup = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; ((changedGroup["proxy-groups"] as Array<Record<string, unknown>>)[0] ?? {}).type = "fallback";
  assert.throws(() => validateShadowParityCandidate(baseObject, changedGroup, fragment, parity), ShadowProfileError);
  const changedProvider = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; ((changedProvider["rule-providers"] as Record<string, Record<string, unknown>>).SSH_Direct_Classical ?? {}).url = "https://example.invalid/changed.yaml";
  assert.throws(() => validateShadowParityCandidate(baseObject, changedProvider, fragment, parity), ShadowProfileError);
  const changedDns = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; (((changedDns.dns as Record<string, unknown>)["nameserver-policy"] as Record<string, string[]>)["geosite:private"] ?? []).push("https://example.invalid/dns-query");
  assert.throws(() => validateShadowParityCandidate(baseObject, changedDns, fragment, parity), ShadowProfileError);
  const changedRule = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; const ssh = (changedRule.rules as string[]).indexOf("RULE-SET,SSH_Direct_Classical,🎯 全球直連"); assert.ok(ssh >= 0); (changedRule.rules as string[])[ssh] = "RULE-SET,SSH_Direct_Classical,DIRECT";
  assert.throws(() => validateShadowParityCandidate(baseObject, changedRule, fragment, parity), ShadowProfileError);
  await assert.rejects(() => composeWithBase(base.replace("RULE-SET,SSH_Direct_Classical,🎯 全球直連", "RULE-SET,Changed,DIRECT")), ShadowProfileError);
  const changedMatch = YAML.parse(baseline.candidateYaml) as Record<string, unknown>; (changedMatch.rules as string[]).push("MATCH,REJECT");
  assert.throws(() => validateShadowParityCandidate(baseObject, changedMatch, fragment, parity), ShadowProfileError);
  const malformed = await mkdtemp(join(tmpdir(), "routing-shadow-malformed-"));
  try {
    const path = join(malformed, "bad.yaml");
    await writeFile(path, "proxy-groups: {}\nrules: []\nrule-providers: {}\nproxy-providers: {}\ndns: {}\n", "utf8");
    await assert.rejects(() => loadLegacyRelaxedBase(path), ShadowProfileError);
  } finally { await rm(malformed, { recursive: true, force: true }); }
  assert.throws(() => validateShadowFragmentShape({ "proxy-groups": {}, "rule-providers": {}, dns: {}, rules: [] }), ShadowProfileError);
  assert.throws(() => validateShadowFragmentShape({ "proxy-groups": [{ type: "select" }], "rule-providers": {}, dns: { "nameserver-policy": {} }, rules: [] }), ShadowProfileError);
});

test("compiler-owned group graph is data-derived for renamed or newly introduced services", () => {
  const fragment = { "proxy-groups": [{ name: "@profile/new-service", type: "select" }, { name: "🆕 New Service", type: "select" }], "rule-providers": {}, dns: { "nameserver-policy": {} }, rules: [] };
  const connected = { "proxy-groups": [{ name: "@profile/new-service", type: "select", proxies: ["🆕 New Service"] }, { name: "🆕 New Service", type: "select", proxies: ["@profile/new-service"] }] };
  assert.doesNotThrow(() => validateCompilerGroupGraph(connected, fragment));
  const orphaned = { "proxy-groups": [{ name: "@profile/new-service", type: "select", proxies: [] }, { name: "🆕 New Service", type: "select", proxies: [] }] };
  assert.throws(() => validateCompilerGroupGraph(orphaned, fragment), ShadowProfileError);
});

async function loadInvalid(name: string): Promise<ReturnType<typeof loadRoutingConfigFromFiles>> {
...
function runShellAction(script: string, action: "--dry-run" | "--reconcile", environment: Readonly<Record<string, string>>): Promise<ChildResult> {
  return new Promise((resolvePromise, rejectPromise) => {
    const child = spawn("sh", [script, action], { cwd: ROOT, env: { ...process.env, ...environment }, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
...
function runShellPreview(script: string, environment: Readonly<Record<string, string>>): Promise<ChildResult> {
...
test("canonical HK matrix and protected Claude configuration validate", async () => {
  ...
  assert.deepEqual(Object.keys(config.services).sort(), [
    "chatgpt", "claude", "copilot", "gemini", "grok", "huggingface", "notebooklm", "perplexity", "poe", "windsurf",
  ]);
...
test("compiler preview resolves the canonical HK service matrix and keeps Claude locked", async () => {
  ...
  const huggingface = plan.services.find((service) => service.id === "huggingface");
  const claude = plan.services.find((service) => service.id === "claude");
  assert.ok(windsurf !== undefined && huggingface !== undefined && claude !== undefined);
  ...
  if (claude.selector.kind === "explicit-node") {
    ...
    assert.deepEqual(
      claude.selector.choices.filter((choice) => choice.kind === "approved-node").map((choice) => choice.node),
      ["US-Claude-01", "US-Claude-02"],
    );
  }
  for (const serviceId of ["copilot", "gemini", "notebooklm", "perplexity", "grok"]) {
    assert.equal(plan.services.find((service) => service.id === serviceId)?.effectiveRoute.id, "direct");
  }
  assert.equal(windsurf.selector.kind, "profile-aware");
  if (windsurf.selector.kind === "profile-aware") {
    assert.equal(JSON.stringify(windsurf.selector.choices).includes("US-Claude-01"), false);
  }
});
...
test("compiler endpoint resolution honors profile endpoint, endpoint, then service routes", async () => {
  ...
  huggingface.endpoints.preview = { ruleset: "AI_HuggingFace_Classical", session: "stateless", routeOverride: "reject" };
  ...
  const compiledHuggingface = plan.services.find((service) => service.id === "huggingface");
  assert.ok(compiledHuggingface !== undefined);
  assert.equal(compiledHuggingface.endpoints.find((endpoint) => endpoint.id === "preview")?.effectiveRoute.id, "us-stable");

  delete hk.endpointOverrides.huggingface;
  const endpointPlan = compileRoutingProfile(mutated, "hk");
  const endpointOnly = endpointPlan.services.find((service) => service.id === "huggingface");
  assert.ok(endpointOnly !== undefined);
  assert.equal(endpointOnly.endpoints.find((endpoint) => endpoint.id === "preview")?.effectiveRoute.id, "reject");
});
...
test("Mihomo projection renders REJECT-first filtered groups, protected rules, and pinned DNS", async () => {
  ...
  assert.deepEqual(fragment.rules.slice(0, 2), ["RULE-SET,AI_Claude_Classical,🔐 Claude Account Guard", "RULE-SET,AI_Claude_Classical,REJECT"]);
  assert.equal(groups.some((group) => group.type === "url-test"), false);
  assert.equal(groups.some((group) => group.name === "🔐 Claude US Pinned"), false);
  assert.equal(groups.some((group) => group.name === "🌍 AI 存取模式"), true);
  assert.deepEqual(groups.filter((group) => group.name.startsWith("@mode/")).map((group) => group.name), ["@mode/hk", "@mode/jp", "@mode/sg", "@mode/us"]);
  ...
  assert.deepEqual(fragment.dns.nameserverPolicy, { "rule-set:AI_Claude_Classical": ["https://1.1.1.1/dns-query#🔐 Claude Account Guard"] });
  assert.equal(fragment.dns.respectRules, true);
  assert.deepEqual(fragment.dns.defaultNameserver, ["1.1.1.1"]);
  assert.deepEqual(fragment.dns.proxyServerNameserver, ["https://1.1.1.1/dns-query"]);
  ...
  assert.match(fragment.ruleProviders.AI_Windsurf_Classical?.url ?? "", /d07cac190c33e7914ba7adaf7e7c14298fba7024\/rules\/clash\/windsurf\.yaml$/);
...
test("Mihomo DNS serializer handles UDP, DoT, and adjacent protected endpoint pairs", async () => {
  ...
  claude.endpoints.extra = { ruleset: "AI_Claude_Extra_Classical", session: "stable" };
  const anthropic = projection.ruleProviders.AI_Claude_Classical;
  assert.ok(anthropic !== undefined);
  projection.ruleProviders.AI_Claude_Extra_Classical = structuredClone(anthropic);
  ...
  const protectedRules = fragment.rules.filter((rule) => rule.startsWith("RULE-SET,AI_Claude"));
...
test("Mihomo projection rejects empty services and duplicate endpoint ruleset identities", async () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "services.claude.endpoints"),
  );
  ...
  huggingface.endpoints.duplicate = { ruleset: "AI_HuggingFace_Classical", session: "stable" };
  assert.throws(
    () => compileMihomoFragment(duplicateRuleset, projection, "hk"),
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "services.huggingface.endpoints.duplicate.ruleset"),
  );
});
...
test("Mihomo projection rejects missing providers and the checked fragment stays deterministic and parseable", async () => {
  ...
  const bindings = missingPinnedBinding.pinnedEgressBindings["claude-us-pinned"];
  assert.ok(bindings !== undefined);
  delete bindings["US-Claude-02"];
  assert.throws(
    () => compileMihomoFragment(config, missingPinnedBinding, "hk"),
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "pinnedEgressBindings.claude-us-pinned"),
  );
  const nonExternalPinnedProvider = structuredClone(projection);
  const nonExternalBindings = nonExternalPinnedProvider.pinnedEgressBindings["claude-us-pinned"];
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "pinnedEgressBindings.claude-us-pinned.US-Claude-01"),
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.code === "missing-reference"),
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "routeTargets.us-stable.group"),
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "ruleProviders.AI_Windsurf_Classical.path"),
  );
  for (const invalidPath of ["rules\\..\\main.yaml", "rules/%2e%2e/main.yaml"]) {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof MihomoProjectionError && error.issues.some((entry) => entry.path.join(".") === "ruleProviders.AI_Windsurf_Classical.source"),
  );
  const unsafeSource = structuredClone(projection);
  const vpsdance = unsafeSource.sources.vpsdance;
  assert.ok(vpsdance !== undefined);
  vpsdance.rawBaseUrl = "https://user@raw.githubusercontent.com/VPSDance/ai-proxy-rules";
  assert.throws(
...
test("compiler rejects semantic invalidity and unknown profile references", async () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof RoutingCompileError && error.issues.some((entry) => entry.code === "policy-invariant"),
  );
  assert.throws(
    () => compileRoutingProfile(config, "missing-profile"),
    (error: unknown) => error instanceof RoutingCompileError && error.issues[0]?.path.join(".") === "accessProfiles.missing-profile",
  );
  assert.throws(
    () => compileRoutingProfile(config, "hk", "missing-dns-profile"),
    (error: unknown) => error instanceof RoutingCompileError && error.issues[0]?.path.join(".") === "dns.profiles.missing-dns-profile",
  );
});
...
test("all canonical profiles have deterministic plan and non-standalone fragment artifacts", async () => {
  ...
  for (const profileId of Object.keys(config.accessProfiles).sort()) {
    ...
    assert.equal(groups.some((group) => String(group.name).endsWith(" Auto")), false);
    assert.equal(groups.some((group) => group.name === "🔐 Claude US Pinned"), false);
    const rules = parsed.rules as string[];
    const deepmind = rules.findIndex((rule) => rule.startsWith("GEOSITE,google-deepmind,"));
    const category = rules.findIndex((rule) => rule.startsWith("GEOSITE,category-ai-!cn,"));
    assert.equal(deepmind >= 0 && category === deepmind + 1, true);
    assert.equal(rules[deepmind]?.split(",").at(-1), rules[category]?.split(",").at(-1));
  }
});
...
test("artifact checker is read-only and rejects missing, stale, and changed outputs", async () => {
  ...
  try {
    for (const [name, content] of expected) await writeFile(join(directory, name), content, "utf8");
    await checkRoutingArtifacts(config, projection, directory);
    await writeFile(join(directory, "hk.full-profile-candidate.yaml"), "shadow owns this artifact\n", "utf8");
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof RoutingArtifactCheckError && error.issues.some((entry) => entry.path.at(-1) === missing && entry.message.includes("missing")),
    );
    assert.equal(await readFile(join(directory, "hk.mihomo-fragment.yaml"), "utf8"), expected.get("hk.mihomo-fragment.yaml"));

    await writeFile(join(directory, missing), expected.get(missing) ?? "", "utf8");
    await writeFile(join(directory, "obsolete.plan.json"), "obsolete\n", "utf8");
    await assert.rejects(
      () => checkRoutingArtifacts(config, projection, directory),
      (error: unknown) => error instanceof RoutingArtifactCheckError && error.issues.some((entry) => entry.path.at(-1) === "obsolete.plan.json" && entry.message.includes("stale")),
    );
    ...
    await assert.rejects(
      () => checkRoutingArtifacts(config, projection, directory),
      (error: unknown) => error instanceof RoutingArtifactCheckError && error.issues.some((entry) => entry.path.at(-1) === "jp.plan.json" && entry.message.includes("differs")),
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
...
test("region filters use boundary-aware codes and a source base path remains pinned", async () => {
  ...
  const toJavaScriptFilter = (filter: string): RegExp => new RegExp(filter.replace(/^\(\?i\)/, ""), "i");
  ...
  source.rawBaseUrl = "https://raw.githubusercontent.com/VPSDance/ai-proxy-rules/base";
  const fragment = compileMihomoFragment(config, withBasePath, "hk");
  assert.match(fragment.ruleProviders.AI_Windsurf_Classical?.url ?? "", /\/base\/d07cac190c33e7914ba7adaf7e7c14298fba7024\/rules\/clash\/windsurf\.yaml$/);
});
...
test("source-owned providers are pinned and canonical services do not invent endpoint roles", async () => {
  ...
  for (const [serviceId, service] of Object.entries(config.services)) {
    ...
    for (const endpoint of Object.values(service.endpoints)) {
      ...
      assert.ok(source !== undefined);
      const fragment = compileMihomoFragment(config, projection, "hk");
      assert.match(fragment.ruleProviders[endpoint.ruleset]?.url ?? "", new RegExp(`/${source.revision}/`));
    }
  }
  assert.equal(projection.ruleProviders.AI_Gemini_Classical?.path, "rule/AI_Gemini_Classical.yaml");
  assert.equal(projection.ruleProviders.AI_NotebookLM_Classical?.path, "rule/AI_NotebookLM_Classical.yaml");
});
...
test("RoutingConfigSchema rejects record IDs outside the canonical ID pattern", async () => {
  ...
  if (!result.success) {
    assert.ok(result.error.issues.some((entry) => entry.path.join(".") === "services.Invalid_ID"));
  }
});

test("RoutingConfigSchema rejects unknown root and nested object keys", async () => {
  const config = await loadRoutingConfig(VALID_DIRECTORY);
  const withUnknownRoot = structuredClone(config) as unknown as Record<string, unknown>;
  ...
  if (!rootResult.success) {
    assert.ok(rootResult.error.issues.some((entry) => entry.message.includes("unexpectedRoot")));
  }
  ...
  if (!nestedResult.success) {
    assert.ok(
      nestedResult.error.issues.some(
        (entry) => entry.path.join(".") === "services.windsurf" && entry.message.includes("unexpectedServiceField"),
      ),
    );
  }
});
...
test("account-protected endpoint overrides fail semantically before compilation", async () => {
  ...
  assert.ok(issues.some((entry) => entry.code === "policy-invariant" && entry.path.join(".") === path && entry.message.includes("account-protected endpoint")));
  assert.throws(
    () => compileRoutingProfile(config, "hk"),
    (error: unknown) => error instanceof RoutingCompileError && error.issues.some((entry) => entry.code === "policy-invariant" && entry.path.join(".") === path),
  );
});
...
test("fragment loader rejects duplicate record IDs instead of overwriting", async () => {
  ...
  try {
    ...
    await writeFile(first, "routeTargets:\n  reject: { kind: reject, group: REJECT }\n", "utf8");
    await writeFile(second, "routeTargets:\n  reject: { kind: reject, group: REJECT }\n", "utf8");
    await assert.rejects(
      () => loadRoutingConfigFromFiles([first, second]),
      (error: unknown) => error instanceof RoutingConfigLoadError && error.issues.some((entry) => entry.code === "duplicate-key"),
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
...
test("fragment loader rejects duplicate YAML mapping keys", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof RoutingConfigLoadError && error.issues.some((entry) => entry.code === "invalid-yaml"),
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
...
test("routing CLI exits non-zero with code, path, and message for invalid input", async () => {
  ...
  try {
    ...
    assert.match(result.stderr, /dns\.profiles\.default\.proxyServerNameserver/);
    assert.match(result.stderr, /respectRules requires at least one proxyServerNameserver/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("every DNS resolver viaRoute must reference a known target", async () => {
  ...
  assert.ok(
    issues.some(
      (entry) => entry.code === "missing-reference" && entry.path.join(".") === "dns.profiles.default.proxyServerNameserver.0.viaRoute",
    ),
  );
});
...
test("account-protected services need proxy-only DNS in every DNS profile", async () => {
  ...
  delete dnsProfile.servicePolicies.claude;
  const issues = validateRoutingSemantics(mutated);
  assert.ok(issues.some((entry) => entry.path.join(".") === "dns.profiles.default.servicePolicies.claude"));
});
...
test("profile-aware services must explicitly override an incompatible profile default", async () => {
  ...
  assert.ok(
    validateRoutingSemantics(mutated).some(
      (entry) => entry.path.join(".") === "accessProfiles.hk.defaultRoute" && entry.message.includes("windsurf"),
    ),
  );
});
...
test("stable/realtime services and non-dynamic protection classes reject selectable auto routes", async () => {
  ...
  assert.ok(
    issues.some(
      (entry) => entry.code === "policy-invariant" && entry.path.join(".").startsWith("services.windsurf.allowedRoutes."),
    ),
  );
  assert.ok(
    issues.some(
      (entry) => entry.code === "dynamic-route" && entry.path.join(".").startsWith("services.windsurf.selector.allowedRouteRefs."),
    ),
  );
});
...
test("pinned egress rejects built-ins and duplicate approved node identities", async () => {
  ...
  const issues = validateRoutingSemantics(mutated);
  assert.ok(issues.some((entry) => entry.path.join(".") === "routeTargets.claude-us-pinned.approvedNodes"));
  assert.ok(issues.some((entry) => entry.path.join(".") === "routeTargets.claude-us-pinned.approvedNodes.2"));
});
...
test("account reset conditions must be unique", async () => {
  ...
  assert.ok(protection !== undefined && protection.kind === "account-protected");
  const first = protection.resetOn[0];
  assert.ok(first !== undefined);
  protection.resetOn.push(first);
  assert.ok(validateRoutingSemantics(mutated).some((entry) => entry.path.join(".") === "protectionClasses.account-protected.resetOn"));
});
...
test("committed JSON Schema is generated from the Zod source", async () => {
  const committed = await readFile(join(ROOT, "schema", "routing-config.schema.json"), "utf8");
...
test("controller plan materializes every access matrix into hidden profile selections only", async () => {
  ...
  assert.deepEqual(Object.keys(plan.modeControl.modes), ["hk", "jp", "sg", "us"]);
  ...
  for (const mode of Object.values(plan.modes)) {
    ...
    assert.ok(mode.hiddenSelections.every((selection) => selection.group.startsWith("@profile/")));
  }
  const claude = plan.accountProtected.find((service) => service.serviceId === "claude");
  assert.deepEqual(claude?.canonicalApprovedNodeIds, ["US-Claude-01", "US-Claude-02"]);
  ...
  assert.equal(claude?.lockRequest.proxyPath, "/proxies/%F0%9F%94%90%20Claude%20Account%20Guard");
  assert.ok(plan.modes.hk?.hiddenSelections.every((selection) => selection.proxyPath.startsWith("/proxies/%40profile%2F")));
  assert.equal(plan.api.requestEncoding, "precomputed-percent-path+jshn-body");
  assert.deepEqual(compileFirewallSemanticPlan(config).deny, ["direct-wan-v4", "direct-wan-v6", "external-dns-tcp-udp-53", "dot-tcp-udp-853", "direct-quic-udp-443"]);
});
...
test("controller plan precomputes one-segment UTF-8 API paths for reserved selector characters", async () => {
  ...
  const account = compileControllerPlan(mutated, projection).accountProtected[0];
  assert.equal(account?.lockRequest.proxyPath, "/proxies/%F0%9F%94%90%20%2F%3F%23%25%20Claude");
});
...
function topologyFixture(): Record<string, unknown> {
  return {
    ...
    mode: "deployment",
    policyVersion: "1",
    protectedSources: { ipv4: ["192.0.2.10/32"], ipv6: ["2001:db8::10/128"], interfaces: ["br-ai"], vlans: ["ai-safe"] },
    routerDns: { ipv4: ["192.0.2.1"], ipv6: ["2001:db8::1"] },
    wanInterfaces: ["pppoe-wan"],
    mihomo: { interceptionChains: ["openclash_mihomo"], routingMark: "0x162", proxyEndpointIps: { ipv4: ["198.51.100.10"], ipv6: ["2001:db8:1::10"] } },
  };
}

test("firewall adapter plan requires exact dual-stack topology and has no runtime defaults", async () => {
  const config = await loadRoutingConfig(VALID_DIRECTORY);
  const plan = compileFirewallAdapterPlan(compileFirewallSemanticPlan(config), topologyFixture());
  ...
  assert.ok(plan.guardRequirements.includes("default-deny-protected-wan-v4-v6"));
  const incomplete = topologyFixture();
  delete (incomplete.protectedSources as Record<string, unknown>).ipv6;
  assert.throws(() => compileFirewallAdapterPlan(compileFirewallSemanticPlan(config), incomplete), RuntimeTopologyError);
  const broad = topologyFixture();
  (broad.protectedSources as Record<string, unknown>).ipv4 = ["192.0.2.0/24"];
  assert.throws(() => compileFirewallAdapterPlan(compileFirewallSemanticPlan(config), broad), RuntimeTopologyError);
});
...
test("account-protected detection follows the protection-class kind rather than a fixed ID", async () => {
  ...
  assert.equal(compileControllerPlan(mutated, projection).accountProtected[0]?.serviceId, "claude");
  const fragment = compileMihomoFragment(mutated, projection, "hk");
...
function deploymentFixture(): Record<string, unknown> {
  return {
    ...
    mode: "deployment",
    policyVersion: "1",
    controller: { url: "http://127.0.0.1:9090", secretFile: "/run/secrets/openclash-controller" },
...
function egressFixture(): Record<string, unknown> {
  return {
    ...
    services: {
      claude: {
        bindings: [
          { approvedId: "US-Claude-01", node: "EXAMPLE-APPROVED-NODE-ONE", provider: "provider1" },
          { approvedId: "US-Claude-02", node: "EXAMPLE-APPROVED-NODE-TWO", provider: "provider1" },
        ],
        revokedNodes: [],
      },
    },
...
function stateFixture(): Record<string, unknown> {
  return {
    ...
    activeMode: "hk",
    accounts: {
      claude: { selectedNode: "REJECT", verifiedPolicyVersion: null, verifiedNode: null, resetReason: "none" },
    },
  };
}
...
test("router-local documents require exact local egress mapping and preserve the initial locked state", async () => {
  ...
  const projection = await loadMihomoProjectionConfig(MIHOMO_PROJECTION);
  const plan = compileControllerPlan(config, projection);
  const result = validateRouterLocalConfig(deploymentFixture(), egressFixture(), stateFixture(), plan);
  assert.equal(result.state.accounts.claude?.selectedNode, "REJECT");

  const wrongVersion = stateFixture();
  wrongVersion.policyVersion = "2";
  assert.throws(() => validateRouterLocalConfig(deploymentFixture(), egressFixture(), wrongVersion, plan), RouterLocalConfigError);

  const missingBinding = egressFixture();
  const claude = missingBinding.services as Record<string, { bindings: unknown[] }>;
  claude.claude?.bindings.pop();
  assert.throws(() => validateRouterLocalConfig(deploymentFixture(), missingBinding, stateFixture(), plan), RouterLocalConfigError);

  const unsafeNode = egressFixture();
  const unsafeClaude = unsafeNode.services as Record<string, { bindings: Array<{ approvedId: string; node: string }> }>;
  const binding = unsafeClaude.claude?.bindings[0];
  if (binding !== undefined) binding.node = "DIRECT";
  assert.throws(() => validateRouterLocalConfig(deploymentFixture(), unsafeNode, stateFixture(), plan), RouterLocalConfigError);
});
...
test("account safety decisions reset stale, revoked, and unverified armed selections to REJECT", async () => {
  ...
  assert.deepEqual(initial.accounts.claude, { selectedNode: "REJECT", verifiedPolicyVersion: null, verifiedNode: null, resetReason: "none" });
  assert.deepEqual(decideAccountSafety(plan, ApprovedEgressSchema.parse(egressFixture()), initial), [{ serviceId: "claude", selection: "REJECT", resetReason: "none" }]);

  const armed = stateFixture();
  const armedAccount = (armed.accounts as Record<string, Record<string, unknown>>).claude;
  ...
  const valid = validateRouterLocalConfig(deploymentFixture(), egressFixture(), armed, plan);
  assert.deepEqual(decideAccountSafety(plan, valid.egress, valid.state), [{ serviceId: "claude", selection: "EXAMPLE-APPROVED-NODE-ONE", resetReason: "none" }]);
  ...
  assert.deepEqual(decideAccountSafety(plan, valid.egress, stale), [{ serviceId: "claude", selection: "REJECT", resetReason: "selected-node-missing" }]);
  ...
  assert.deepEqual(decideAccountSafety(plan, revoked, valid.state), [{ serviceId: "claude", selection: "REJECT", resetReason: "node-revoked" }]);
  const changedPlan = { ...plan, policyVersion: "2" as const };
  assert.deepEqual(decideAccountSafety(changedPlan, valid.egress, valid.state), [{ serviceId: "claude", selection: "REJECT", resetReason: "policy-version-change" }]);
});

test("materialized account graph accepts only REJECT or exact approved nodes", () => {
  validateAccountMaterializedGraph("🔐 Claude Account Guard", ["EXAMPLE-APPROVED-NODE-ONE"], {
    groups: { "🔐 Claude Account Guard": ["REJECT", "EXAMPLE-APPROVED-NODE-ONE"] },
  });
  assert.throws(() => validateAccountMaterializedGraph("🔐 Claude Account Guard", ["EXAMPLE-APPROVED-NODE-ONE"], {
    groups: { "🔐 Claude Account Guard": ["DIRECT"] },
  }), RouterLocalConfigError);
  assert.throws(() => validateAccountMaterializedGraph("🔐 Claude Account Guard", ["EXAMPLE-APPROVED-NODE-ONE"], {
    groups: { "🔐 Claude Account Guard": ["US Auto"], "US Auto": ["EXAMPLE-APPROVED-NODE-ONE"] },
  }), RouterLocalConfigError);
});
...
test("effective cutover proof requires account DNS selector, startup gate, and frozen legacy authority", async () => {
  ...
  const projection = await loadMihomoProjectionConfig(MIHOMO_PROJECTION);
  const plan = compileControllerPlan(config, projection);
  const local = validateRouterLocalConfig(deploymentFixture(), egressFixture(), stateFixture(), plan);
  const proof = {
    schemaVersion: 1,
    policyVersion: "1",
    ...
    proxyStates: { "🔐 Claude Account Guard": { type: "Selector", all: ["REJECT", "EXAMPLE-APPROVED-NODE-ONE", "EXAMPLE-APPROVED-NODE-TWO"], now: "REJECT", emptyFallback: "REJECT", udp: true } },
    dnsRuntime: { source: "mihomo-running-config", policies: { "rule-set:AI_Claude_Classical": { selector: "🔐 Claude Account Guard", fallback: false } } },
    startup: { storeSelectedDeclared: true, accountGate: "locked-until-explicit-selection" },
    legacyShellEnforcement: "frozen-non-authoritative",
  };
  assert.equal(validateEffectiveCutover(plan, local.deployment, local.egress, proof).policyVersion, "1");
  proof.dnsRuntime.policies["rule-set:AI_Claude_Classical"].fallback = true;
  assert.throws(() => validateEffectiveCutover(plan, local.deployment, local.egress, proof), EffectiveCutoverError);
});
...
class FakeControllerApi implements ControllerApi {
  ...
  public constructor(
    ...
    this.sequentialReadbacks = new Map(
      [...sequentialReadbacks.entries()].map(([group, values]) => [group, [...values]]),
    );
  }
  ...
  public async selectedProxy(group: string): Promise<string> {
    ...
    return this.readbackOverrides.get(group) ?? this.selections.get(group) ?? "REJECT";
  }
...
async function controllerFixture(): Promise<{
  ...
  const plan = compileControllerPlan(config, projection);
  const local = validateRouterLocalConfig(deploymentFixture(), egressFixture(), stateFixture(), plan);
  return { plan, local, preview: previewControllerTransaction(plan, local.state) };
}

function firstHiddenUpdate(preview: ReturnType<typeof previewControllerTransaction>): { readonly group: string; readonly target: string } {
  const operation = preview.find((item) => item.phase === "hidden-update");
  if (operation?.target === undefined) throw new Error("expected a hidden profile operation");
...
function assertRedactedControllerFailure(
  ...
  assert.doesNotMatch(diagnostic, /SENTINEL_API_SECRET|SENTINEL_GATE_SECRET|SENTINEL_APPROVED_NODE/);
  return true;
}
...
test("controller startup gate fails before any controller API call and redacts gate detail", async () => {
  ...
  const api = new FakeControllerApi(new Map([
  ...
  await assert.rejects(
    () => executeControllerTransaction(api, local.deployment, plan, local.state, failingGate),
    (error: unknown) => assertRedactedControllerFailure(error, "startup-gate", "startup-gate-failed", 0),
  );
...
test("controller locks and verifies account guards before hidden writes without changing visible services", async () => {
  ...
  assert.ok(preview.filter((item) => item.phase === "hidden-update").every((item) => item.group.startsWith("@profile/")));
  assert.equal(preview.some((item) => item.group === "🌊 Windsurf"), false);
  const previewRead = preview.find((item) => item.phase === "hidden-read" && item.group === firstHidden.group);
  const previewWrite = preview.find((item) => item.phase === "hidden-update" && item.group === firstHidden.group);
  const previewReadback = [...preview].reverse().find((item) => item.phase === "verify" && item.group === firstHidden.group);
  assert.ok(previewRead !== undefined && previewWrite !== undefined && previewReadback !== undefined);
  ...
  const result = await executeControllerTransaction(api, local.deployment, plan, local.state, passingStartupGate);
  ...
  assert.ok(api.transcript.indexOf(accountLock) < api.transcript.indexOf(accountReadback));
  assert.ok(api.transcript.indexOf(accountReadback) < api.transcript.indexOf(firstHiddenRead));
  assert.ok(api.transcript.indexOf(`PUT ${firstHidden.group}=${firstHidden.target}`) < api.transcript.lastIndexOf(firstHiddenRead));
  assert.equal(api.transcript.some((line) => line.startsWith("PUT 🌊 Windsurf=")), false);
  assert.equal(api.selectionFor("🔐 Claude Account Guard"), "REJECT");
  assert.equal(result.rolledBack, false);
  assert.ok(result.operations.some((operation) => operation.phase === "verify" && operation.group === firstHidden.group));
  assertRedactedControllerTranscript(api);
});
...
test("controller redacts account API and readback failures before hidden writes", async () => {
  const { plan, local, preview } = await controllerFixture();
  const firstHidden = firstHiddenUpdate(preview);
  const failingApi = new FakeControllerApi(new Map([
    ["🔐 Claude Account Guard", "SENTINEL_APPROVED_NODE"],
    [firstHidden.group, "DIRECT"],
  ]), ["🔐 Claude Account Guard=REJECT"]);

  await assert.rejects(
    () => executeControllerTransaction(failingApi, local.deployment, plan, local.state, passingStartupGate),
    (error: unknown) => assertRedactedControllerFailure(error, "account-lock", "api-call-failed", 0),
  );
  assert.equal(failingApi.transcript.some((line) => line.startsWith("GET @profile/")), false);
  ...
  await assert.rejects(
    () => executeControllerTransaction(mismatchApi, local.deployment, plan, local.state, passingStartupGate),
    (error: unknown) => assertRedactedControllerFailure(error, "account-readback", "readback-mismatch", 0),
  );
  assert.equal(mismatchApi.transcript.some((line) => line.startsWith("GET @profile/")), false);
...
test("controller rolls back hidden selections only and preserves account locks on hidden failure", async () => {
  ...
  await assert.rejects(
    () => executeControllerTransaction(api, local.deployment, plan, local.state, passingStartupGate),
    (error: unknown) => assertRedactedControllerFailure(error, "hidden-update", "api-call-failed", 0),
  );
  assert.equal(api.transcript.at(-1), `PUT ${firstHidden.group}=DIRECT`);
  assert.equal(api.transcript.filter((line) => line === "PUT 🔐 Claude Account Guard=REJECT").length, 1);
  assert.equal(api.transcript.some((line) => line.startsWith("PUT 🌊 Windsurf=")), false);
  assertRedactedControllerTranscript(api);

  const rollbackFailure = new FakeControllerApi(new Map([
    ["🔐 Claude Account Guard", "SENTINEL_APPROVED_NODE"],
    [firstHidden.group, "DIRECT"],
  ]), [`${firstHidden.group}=${firstHidden.target}`, `${firstHidden.group}=DIRECT`]);
  await assert.rejects(
    () => executeControllerTransaction(rollbackFailure, local.deployment, plan, local.state, passingStartupGate),
    (error: unknown) => assertRedactedControllerFailure(error, "hidden-update", "api-call-failed", 1),
  );
...
test("controller rolls back a hidden selector when its post-write readback mismatches", async () => {
  ...
  await assert.rejects(
    () => executeControllerTransaction(api, local.deployment, plan, local.state, passingStartupGate),
    (error: unknown) => assertRedactedControllerFailure(error, "hidden-readback", "readback-mismatch", 0),
  );
  const write = `PUT ${firstHidden.group}=${firstHidden.target}`;
  const rollback = `PUT ${firstHidden.group}=${previous}`;
  assert.ok(api.transcript.indexOf(`GET ${firstHidden.group}`) < api.transcript.indexOf(write));
  assert.ok(api.transcript.indexOf(write) < api.transcript.lastIndexOf(`GET ${firstHidden.group}`));
...
test("controller restores every recorded hidden selector in reverse order after a later hidden write failure", async () => {
  ...
  const hidden = preview.filter(
    (operation): operation is { readonly phase: "hidden-update"; readonly group: string; readonly target: string } =>
      operation.phase === "hidden-update" && operation.target !== undefined,
  );
  assert.ok(hidden.length >= 2);
  const [firstHidden, secondHidden] = hidden;
  if (firstHidden === undefined || secondHidden === undefined) throw new Error("expected two hidden profile operations");
...
test("POSIX controller preview consumes line-preserving jsonfilter output and rejects unsupported IPv6 spelling", async () => {
  const directory = await mkdtemp(join(tmpdir(), "routing-controller-preview-"));
  try {
    ...
    await new Promise<void>((resolvePromise, rejectPromise) => {
      spawn("mkdir", ["-p", tools]).once("error", rejectPromise).once("close", (code) => code === 0 ? resolvePromise() : rejectPromise(new Error("mkdir failed")));
    });
    await Promise.all([writeFile(plan, "{}\n"), writeFile(deployment, "{}\n"), writeFile(egress, "{\"node\":\"EXAMPLE node 🤖 with spaces\"}\n"), writeFile(state, "{}\n"), writeFile(secret, "example-token-never-printed\n")]);
    await writeFile(curl, "#!/bin/sh\ncase \"$*\" in *'/proxies'*) printf '%s\\n' 'GET /proxies' > \"$AI_ROUTING_TEST_CURL_LOG\" ;; *) exit 2 ;; esac\n", "utf8");
    await writeFile(jshn, "json_init() { :; }\njson_add_string() { :; }\njson_dump() { printf '%s' '{}'; }\n", "utf8");
    const fakeJsonfilter = [
      "#!/bin/sh", "expr=", "while [ $# -gt 0 ]; do", "  if [ $1 = -e ]; then expr=$2; shift 2; else shift; fi", "done", "case $expr in",
      "  '@.policyVersion') printf '%s\\n' '1' ;;", "  '@.controller.url') printf '%s\\n' ${AI_ROUTING_TEST_URL:-http://127.0.0.1:9090} ;;", "  '@.controller.secretFile') printf '%s\\n' $AI_ROUTING_TEST_SECRET ;;", "  '@.activeMode') printf '%s\\n' 'hk' ;;",
      "  '@.accountProtected[*].visibleGroup') printf '%s\\n' '🔐 Claude Account Guard' ;;", "  '@.modes.hk.hiddenSelections[#]') printf '%s\\n' '2' ;;", "  '@.modes.hk.hiddenSelections[0].group') printf '%s\\n' '@profile/windsurf' ;;", "  '@.modes.hk.hiddenSelections[0].target') printf '%s\\n' '🇺🇸 US Stable' ;;", "  '@.modes.hk.hiddenSelections[1].group') printf '%s\\n' '@profile/huggingface' ;;", "  '@.modes.hk.hiddenSelections[1].target') printf '%s\\n' 'DIRECT' ;;", "  *) exit 2 ;;", "esac", "",
    ].join("\n");
    await writeFile(jsonfilter, fakeJsonfilter, "utf8");
    await Promise.all([chmod(curl, 0o755), chmod(jsonfilter, 0o755)]);
    const environment = { PATH: `${tools}:${process.env.PATH ?? ""}`, AI_ROUTING_PLAN: plan, AI_ROUTING_DEPLOYMENT: deployment, AI_ROUTING_EGRESS: egress, AI_ROUTING_STATE: state, AI_ROUTING_JSHN: jshn, AI_ROUTING_TEST_SECRET: secret, AI_ROUTING_TEST_CURL_LOG: curlLog };
    const preview = await runShellPreview(join(ROOT, "shell", "ai-routing-controller.sh"), environment);
    assert.equal(preview.exitCode, 0, preview.stderr);
    assert.match(preview.stdout, /reconciliation is disabled/);
    assert.match(preview.stdout, /no controller API, secret file, local binding, or runtime-state input is read/);
...
test("POSIX reconcile is an explicit fail-closed stub and performs no API calls", async () => {
  ...
  try {
    ...
    const curl = join(tools, "curl");
    const jshn = join(tools, "jshn.sh");
    await Promise.all([writeFile(plan, "{}"), writeFile(deployment, "{}"), writeFile(egress, "{}"), writeFile(state, "{}"), writeFile(secret, "non-newline-secret")]);
    await writeFile(jshn, ["json_init() { :; }", "json_add_string() { :; }", "json_dump() { printf '%s' '{\\\"name\\\":\\\"safe\\\"}'; }", ""].join("\n"));
    await writeFile(jsonfilter, [
      "#!/bin/sh", "file=", "expr=", "while [ $# -gt 0 ]; do case $1 in -i) file=$2; shift 2 ;; -e) expr=$2; shift 2 ;; *) shift ;; esac; done", "input=$(cat ${file:-/dev/stdin})", "case $expr in",
      "  '@.policyVersion') printf '%s\\n' 1 ;;", "  '@.controller.url') printf '%s\\n' http://127.0.0.1:9090 ;;", "  '@.controller.secretFile') printf '%s\\n' $AI_ROUTING_RECONCILE_SECRET ;;", "  '@.api.versionPath') printf '%s\\n' /version ;;", "  '@.version') printf '%s\\n' 1.19.0 ;;", "  '@.accountProtected[#]') printf '%s\\n' 1 ;;", "  '@.accountProtected[0].lockRequest.proxyPath') printf '%s\\n' /proxies/%F0%9F%94%90%20Claude%20Account%20Guard ;;", "  '@.accountProtected[0].lockRequest.target') printf '%s\\n' REJECT ;;", "  '@.activeMode') printf '%s\\n' hk ;;", "  '@.modes.hk.hiddenSelections[#]') printf '%s\\n' 1 ;;", "  '@.modes.hk.hiddenSelections[0].group') printf '%s\\n' @profile/windsurf ;;", "  '@.modes.hk.hiddenSelections[0].proxyPath') printf '%s\\n' /proxies/%40profile%2Fwindsurf ;;", "  '@.modes.hk.hiddenSelections[0].target') printf '%s\\n' DIRECT ;;", "  '@.type') printf '%s\\n' Selector ;;", "  '@.now') case $input in *HIDDEN*) printf '%s\\n' DIRECT ;; *) printf '%s\\n' REJECT ;; esac ;;", "  '@.all[*]') printf '%s\\n' REJECT; printf '%s\\n' DIRECT ;;", "  *) exit 2 ;;", "esac", "",
    ].join("\n"));
    await writeFile(curl, ["#!/bin/sh", "printf '%s\\n' \"$*\" >> \"$AI_ROUTING_RECONCILE_TRANSCRIPT\"", "case \"$*\" in *--request*PUT*) printf 204 ;; *%40profile%2Fwindsurf*) printf '%s' HIDDEN ;; *) printf '%s' ACCOUNT ;; esac", ""].join("\n"));
    await Promise.all([chmod(jsonfilter, 0o755), chmod(curl, 0o755)]);
    const env = { PATH: `${tools}:${process.env.PATH ?? ""}`, AI_ROUTING_PLAN: plan, AI_ROUTING_DEPLOYMENT: deployment, AI_ROUTING_EGRESS: egress, AI_ROUTING_STATE: state, AI_ROUTING_JSHN: jshn, AI_ROUTING_RECONCILE_SECRET: secret, AI_ROUTING_RECONCILE_TRANSCRIPT: transcript };
    const result = await runShellAction(join(ROOT, "shell", "ai-routing-controller.sh"), "--reconcile", env);
    assert.notEqual(result.exitCode, 0);
...
test("tampered account lock targets and paths never reach the disabled controller API", async () => {
  ...
  try {
    ...
    for (const tamperedLock of [
      ...
      await writeFile(plan, JSON.stringify({ accountProtected: [{ visibleGroup: "🔐 Claude Account Guard", lockRequest: tamperedLock }] }));
      const result = await runShellAction(join(ROOT, "shell", "ai-routing-controller.sh"), "--reconcile", { PATH: `${directory}:${process.env.PATH ?? ""}`, AI_ROUTING_PLAN: plan });
      assert.notEqual(result.exitCode, 0);
      assert.match(result.stderr, /reconcile is disabled/);
    }
    await assert.rejects(() => readFile(transcript, "utf8"));
...
test("private materializer preserves the candidate except allowed private deltas and never reports secrets or nodes", async () => {
  ...
  try {
    ...
    const deployment = { ...deploymentFixture(), controller: { url: "http://127.0.0.1:9090", secretFile: secret } };
    const egress = egressFixture();
    const bindings = ((egress.services as Record<string, { bindings: Array<{ approvedId: string; node: string; provider: string }> }>).claude?.bindings);
    assert.ok(bindings !== undefined);
    ...
    const rendered = YAML.parse(await readFile(output, "utf8")) as Record<string, unknown>;
    const groups = rendered["proxy-groups"] as Array<Record<string, unknown>>;
    const account = groups.find((group) => group.name === "🔐 Claude Account Guard");
    assert.deepEqual(account?.proxies, ["REJECT"]);
    assert.deepEqual(account?.use, ["provider1"]);
    assert.equal(account?.filter, "^(?:節點 A \\+ \\(safe\\)|node \\[B\\]\\?)$");
    ...
    const normalizedGroup = (normalized["proxy-groups"] as Array<Record<string, unknown>>).find((group) => group.name === "🔐 Claude Account Guard");
...
test("private materializer rejects shape drift, duplicates, unsafe paths, and cleans failed atomic output", async () => {
  const directory = await mkdtemp(join(tmpdir(), "private-materializer-reject-"));
  try {
    const local = join(directory, "local", "ai-routing");
    await mkdir(local, { recursive: true });
    await chmod(local, 0o700);
    ...
    const deployment = { ...deploymentFixture(), controller: { url: "http://127.0.0.1:9090", secretFile: secret } };
    const egress = egressFixture();
    const run = async (value: Record<string, unknown>, output = join(local, "out.yaml")): Promise<void> => { await writeFile(candidate, YAML.stringify(value)); await materializePrivateProfile(candidate, output, plan, deployment, egress, options); };
    const account = (original["proxy-groups"] as Array<Record<string, unknown>>).find((group) => group.name === "🔐 Claude Account Guard"); assert.ok(account !== undefined);
    const duplicate = structuredClone(original); (duplicate["proxy-groups"] as Array<unknown>).push(structuredClone(account));
    await assert.rejects(() => run(duplicate), PrivateMaterializerError);
    const drift = structuredClone(original); const driftGroup = (drift["proxy-groups"] as Array<Record<string, unknown>>).find((group) => group.name === "🔐 Claude Account Guard"); assert.ok(driftGroup !== undefined); driftGroup.use = ["provider1"];
    await assert.rejects(() => run(drift), PrivateMaterializerError);
    await assert.rejects(() => run(original, join(directory, "outside.yaml")), PrivateMaterializerError);
    await assert.rejects(() => run(original, join(local, "..", "..", "evil.yaml")), PrivateMaterializerError);
    await assert.rejects(() => materializePrivateProfile(candidate, candidate, plan, deployment, egress, options), PrivateMaterializerError);
    const outputDirectory = join(local, "existing-output"); await mkdir(outputDirectory);
    await assert.rejects(() => run(original, outputDirectory));
    assert.equal((await stat(outputDirectory)).isDirectory(), true);
    assert.equal((await (await import("node:fs/promises")).readdir(local)).some((name) => name.startsWith(".materialize-")), false);
    const duplicateNode = structuredClone(egress); const bindings = ((duplicateNode.services as Record<string, { bindings: Array<{ node: string }> }>).claude?.bindings); assert.ok(bindings !== undefined); const first = bindings[0]; const second = bindings[1]; assert.ok(first !== undefined && second !== undefined); second.node = first.node;
    await writeFile(candidate, YAML.stringify(original));
    await assert.rejects(() => materializePrivateProfile(candidate, join(local, "duplicate-node.yaml"), plan, deployment, duplicateNode, options), PrivateMaterializerError);
    ...
    await assert.rejects(() => materializePrivateProfile(candidate, join(local, "malicious-rules.yaml"), plan, deployment, egress, options), (error: unknown) => error instanceof PrivateMaterializerError && error.issues.some((entry) => entry.path.join(".") === "candidate"));
    const maliciousDns = structuredClone(original);
    ((maliciousDns.dns as Record<string, unknown>).nameserver as unknown[]).push("https://example.invalid/dns-query");
    await writeFile(candidate, YAML.stringify(maliciousDns));
    await assert.rejects(() => materializePrivateProfile(candidate, join(local, "malicious-dns.yaml"), plan, deployment, egress, options), PrivateMaterializerError);
    const maliciousProvider = structuredClone(original);
    ((maliciousProvider["proxy-providers"] as Record<string, Record<string, unknown>>).provider1 as Record<string, unknown>).type = "file";
    await writeFile(candidate, YAML.stringify(maliciousProvider));
    await assert.rejects(() => materializePrivateProfile(candidate, join(local, "malicious-provider.yaml"), plan, deployment, egress, options), PrivateMaterializerError);
    await writeFile(candidate, await readFile(source));
    const escaped = join(directory, "escaped"); await mkdir(escaped); await symlink(escaped, join(local, "escape"));
    await assert.rejects(() => materializePrivateProfile(candidate, join(local, "escape", "private.yaml"), plan, deployment, egress, options), PrivateMaterializerError);
    const trusted = join(directory, "trusted"); const escapedRoot = join(directory, "escaped-root");
...
test("materialize-private CLI returns structured redacted failure output", async () => {
  ...
  try {
    ...
    await writeFile(secret, "SENTINEL_SECRET_DO_NOT_PRINT"); await chmod(secret, 0o600);
    ...
    const result = await new Promise<ChildResult>((resolvePromise, rejectPromise) => {
      ...
      child.once("error", rejectPromise); child.once("close", (exitCode) => resolvePromise({ exitCode, stdout, stderr }));
    });
    assert.notEqual(result.exitCode, 0);
    assert.match(result.stderr, /\[policy-invariant\].*local/);
    assert.doesNotMatch(`${result.stdout}${result.stderr}`, /SENTINEL_SECRET_DO_NOT_PRINT|SENTINEL_NODE_DO_NOT_PRINT/);
  } finally { await rm(directory, { recursive: true, force: true }); }
});
